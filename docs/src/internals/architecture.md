...
The existing sandbox-ID HTTP endpoints still observe activation when entering the orchestrator. They cannot distinguish a request delayed before API arrival. The explicit `pause_sandbox_for_activation` and `delete_sandbox_for_activation` methods provide the boundary for the pending KE-04 API/controller integration; that integration must carry the caller's expected activation and qualify response-loss and stale-request behavior before execution-authority cutover. The current ID-only API and its Cloud lifecycle callers remain the named temporary consumers, removed when KE-04–08 integrate the explicit precondition. These checks neither grant execution nor prove physical cleanup.
