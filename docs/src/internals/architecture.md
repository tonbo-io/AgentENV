...
### Lifecycle activation preconditions

The orchestrator's pause and delete transitions compare the expected funded activation with current metadata under the same store write lock that acquires the transitional state. Unfunded execution is an exact `None` identity, not a wildcard. Delete retries retain the original expected activation across waits; a later resume cannot silently become the deletion target. Expiry and shutdown workers pass the activation from their observed metadata. Concurrent pause completion also validates activation before reporting success.

The pause and delete HTTP endpoints accept `expectedActivationID` from the caller and return 409 on mismatch. They dispatch through `pause_sandbox_for_activation` and `delete_sandbox_for_activation`. Calls omitting this parameter still observe activation at arrival and cannot distinguish a request delayed before API arrival. Pending KE-04 Cloud/controller integration must supply the precondition and qualify response-loss and stale-request behavior before execution-authority cutover. The current ID-only API and its Cloud lifecycle callers remain the named temporary consumers, removed when KE-04–08 integrate the explicit precondition. These checks neither grant execution nor prove physical cleanup.

### Unadmitted activation revocation

Node admission can durably prevent a never-admitted activation from acquiring execution, using its existing synchronous activation registry and the same claim lock as acquire. A revoked-before-admission record returns `NeverAdmitted` on replay, including after reopening the store. Existing sandbox-ID values remain admission history and return `PreviouslyAdmitted`; that result proves neither runtime presence nor physical stop. Records bind the activation to the exact sandbox identity. The revocation task retains the claim lock through the write after caller cancellation. `POST /nodes/{nodeID}/activation-revocations` carries cluster, service-instance, sandbox and activation identities, rejecting a different node incarnation before accessing admission. The response echoes those identities and the admission disposition. This API is addressed directly to the selected node, like the existing drain client. Concurrent acquire/revoke qualification and Cloud recovery consumption remain pending; this internal primitive alone is not a cleanup receipt or a cross-node fence.

### Conditional node drain

`POST /nodes/{nodeID}/drain` accepts `onlyIfIdle: true` for an empty or fully paused node. The orchestrator snapshots the node-admission generation before reading its complete local metadata inventory, then verifies that generation and zero in-flight starts under the same mutex as create, resume and fork admission before persisting closure. Every admitted start advances the generation, including a start that finishes during the inventory read. Running, transitional or cleanup-pending metadata, unknown inventory and an intervening admission never qualify as idle. A busy request returns HTTP 412 without changing admission or writing a drain marker. Fully paused guests still require durable publication and detach; admission closure alone never authorizes host termination.

The existing unconditional form remains the planned-evacuation mechanism used by explicit drain callers. Both forms share the same durable marker and replay semantics. Retry the same drain ID after a lost response; after restart use the new service instance ID while retaining that drain ID. A persistence failure keeps admission closed, and a conflicting drain ID cannot replace the marker. Idle windows, victim selection, capacity bounds, funding and provider termination remain the owning controller's responsibility.

### Activation-fenced terminal guest command

`DELETE /sandboxes/{id}` optionally accepts `terminalCommand`, an absolute guest executable path without arguments. This option requires a nonzero `expectedActivationID`. The orchestrator first acquires the same activation-checked `Killing` transition used for deletion and persists deletion intent; only then may the running backend execute the command. The command is bounded to 30 seconds and failure does not veto physical deletion. Paused guests are never resumed to execute it. Concurrent resume cannot pass the terminal state, and cancellation of the HTTP caller does not cancel the owned deletion operation. The existing plain-delete path remains used by internal cleanup, CLI and callers without a guest shutdown requirement; the Cloud lifecycle caller must replace its separate unfenced guest exec only after this Node API passes isolated EKS qualification and is deployed.

### Retained paused snapshot publication

`POST /sandboxes/{sandboxID}/paused-snapshots` publishes already retained paused artifacts with required nonzero `snapshotId` and `expectedActivationID`. It does not create a backend VM, start a guest or request a funding lease. The orchestrator acquires an activation-bound Snapshotting transition from Paused and keeps it for the entire repository publication; resume/delete cannot change or remove source artifacts meanwhile. The persisted paused record stays intact. Repository errors and publisher panic restore Paused; HTTP caller cancellation does not cancel the owned publication. A process restart recovers the persisted Paused record. A committed replay returns only a snapshot with the same source Sandbox, activation and alias, including after the source is deleted. A new publication rejects running or superseded activations.

Snapshot Sandbox sources persist optional activation attribution. Existing committed snapshots are the named consumer of the absent-field read path: they remain readable and match only the legacy capture replay contract. Paused publication requires explicit attribution and never accepts an old unattributed record as its replay. Retain this reader until those committed snapshots have expired or been explicitly migrated; do not rewrite stored customer snapshots as part of deployment. OpenAPI owns the new wire request, and SnapshotRecord owns replay comparison across API and repository manager.

This Node primitive is required for paused Machine evacuation, but does not itself transfer Machine placement, delete the source or authorize node termination. The Kubernetes execution controller must durably record the committed artifact and ownership handoff before source cleanup, preserve Paused without a new funded activation, and prove later resume continuity. Isolated EKS publication/failure/restart and full controller transfer acceptance remain required.
