...
| Tools drive | `<deps_path>/tools/{version}/tools.ext4` for fresh launch; snapshot repository `managed-layers/sha256:*` for durable restore | Read-only ext4 image with static `agentenv-init` and `envd` binaries | Fresh sandboxes use the immutable installed release; new snapshots publish and pin the exact bytes by content digest so another node can restore them | Setup extracts or imports the current version. Snapshot publication imports the exact file into the content-addressed repository; restore validates the digest and treats the node-local copy only as an evictable cache. Shared managed layers are retained independently of individual snapshot records until a future reference-aware GC is introduced. |
...
| ---------------------------- | ------------------------------------------------------------ | --------------------------------------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| Managed live snapshot root   | `<firecracker-work-base>/managed-snapshots/{sandbox_id}/{uuid}/...` | Firecracker snapshot artifacts kept alive in-process      | Holds captured running-sandbox snapshots until publish, and supports in-process pause state | Created by `FirecrackerSandbox::pause()` or `capture_snapshot()`. Removed when `PersistentSnapshotRootGuard` drops. | No.                                                          |
