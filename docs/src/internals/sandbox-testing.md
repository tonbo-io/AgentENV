...
## Pressure-aware placement consumers

The gateway accepts an exact `placement.nodeID` alongside the existing `differentNodeFrom` and `snapshotCompatibleWith` constraints. A required node is filtered by ordinary resource admission and compatibility; it never falls back to another node. `/nodes` includes `reportedAtUnixMs` and an opaque `snapshotCompatibilityKey`. The key and Scheduler compatibility filtering share one implementation; an empty key means incomplete identity. Authenticated Sandbox control responses carry gateway-owned `x-agentenv-node-id`, including usage responses, so an external policy controller can associate a runtime meter with the physical placement that served it. Application proxy responses do not acquire this control-plane contract.
