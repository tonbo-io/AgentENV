...
- `scheduler.leader_election.enabled` runs full Scheduler replicas as a single-active Kubernetes Lease group. It requires Redis-backed bindings, Kubernetes discovery, `lease_name`, `lease_namespace`, a unique per-Pod `identity`, and durations accepted by client-go (`lease_duration > renew_deadline > 1.2 * retry_period > 0`). Followers report the named Scheduler gRPC health service as not serving and reject every non-health RPC.
- With leader election enabled, the global gRPC health service (`""`) is process liveness and remains `SERVING` on followers; `scheduler.v1.Scheduler` is leadership health and is `SERVING` only on the leader. Kubernetes liveness and controller readiness may check global health when a Service uses the published leader label for endpoint membership; direct leader-only readiness checks may check the named service.
- `scheduler.leader_election.service_label_key` and `service_label_value` optionally publish Service membership on the elected Pod. The Scheduler removes the label before joining election after a container restart, publishes it before opening the leadership gate, and removes it after fencing RPCs. The ServiceAccount needs `patch` on its own Pod, and the leader Service selector must require the configured label while replica controllers use global process health.
...
- `POST /sandboxes` accepts composable hard placement constraints. `placement.differentNodeFrom` provides node anti-affinity, while `placement.snapshotCompatibleWith` requires a fresh READY target with the same AgentENV release, cluster, CPU architecture, family, model, and Firecracker CPU configuration as every referenced Sandbox. Referenced bindings must exist and remain live; missing references or an unsatisfiable constraint fail closed. The gateway consumes these scheduler-only fields before proxying; a direct runtime-node request containing a placement object is rejected. Cross-node snapshot relocation combines both constraints, while replica anti-affinity uses only `differentNodeFrom`.
...
- For high availability, run at least two full Scheduler replicas with Kubernetes leader election, leader-label Service routing, and Redis-backed bindings, plus multiple query-only replicas started with `--query-only` against the same Redis. Point runtime nodes and gateway control-plane calls at the leader-label Service, and point `gateway.query_only_scheduler_addr` at the query-only Service. Existing-sandbox `LookupNode` remains available during leader replacement; new scheduling resumes after a follower acquires the Lease and receives fresh READY node heartbeats. Observed-node state and the P2P artifact index are intentionally ephemeral, so followers do not serve control RPCs and a new leader does not place work from a stale resource view.
...
Drain observations also report `inFlightOperations` for all cancellation-safe lifecycle tasks and `interruptedOperations` for tasks that exited without returning a result. HTTP caller cancellation does not release an executing task's count. A nonzero interrupted count requires recovery; a new service instance with zero counters does not establish that its predecessor finished physical cleanup. These counters complement runtime inventory and exact-instance fencing and are not a standalone termination gate.

The drain observation includes `sandboxIDs`, the complete local metadata inventory, including paused and cleanup-pending entries. Consumers must match these identities to their execution records and unresolved operations; aggregate counts alone are insufficient for evacuation. Metadata inventory is not a proof that all physical kernel resources are gone.
