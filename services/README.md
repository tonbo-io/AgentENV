...
- With leader election enabled, the global gRPC health service (`""`) is process liveness and remains `SERVING` on followers; `scheduler.v1.Scheduler` is leadership readiness and is `SERVING` only on the leader. Kubernetes liveness probes must check global health and readiness probes must check the named Scheduler service.
