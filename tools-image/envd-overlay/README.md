...
The Linux `ProcessGroup` backend creates a private cgroup v2 child under the existing envd process-type group, with a descriptor intended for `CLONE_INTO_CGROUP` at process birth. It checks the actual filesystem type and refuses to adopt an existing incarnation directory. Freeze/thaw waits for the kernel's `cgroup.events` acknowledgement, with a ten-second upper bound; a timeout never automatically thaws the group. Cleanup refuses to remove populated groups and retains ownership for a later retry. This covers descendants inheriting cgroup membership, including new sessions; it is not a security boundary against a privileged guest moving itself out of its cgroup.

Recoverable process launch uses this private group when envd has a cgroup manager, including PTY creation. A guest without that manager retains ordinary recoverable I/O but cannot prepare a handoff. Group creation failure never falls back to the shared group. Failed starts and exited parents retain their bounded recovery reservation until the kernel permits group cleanup; descendants that outlive the parent cannot be forgotten just because journal retention expired.

The same generated process service exposes `PrepareHandoff` and `ResumeHandoff`, accepting only an incarnation selector, consecutive nonzero epoch and immutable operation ID. A successful response echoes that identity after physical acknowledgement. Exact retries preserve receipts; stale/conflicting handoffs fail closed. Both Go messages and Connect handlers are generated from this schema (pinned `protoc-gen-connect-go` v1.18.1), as is the Rust client. The outer runtime owner must persist the handoff identity before preparation, atomically snapshot with the source paused, fence the source, repair the target, and commit the target's execution authority before requesting resume. Guest transport access alone is not proof of those external conditions.

The snapshot/restore adapter, target authorization and Cloud consumer still need implementation and physical qualification. A callback must prove the entire process tree quiescent and must not reenter the journal. A successful callback cannot be inferred from lease expiry or a stopped parent PID. No serving handoff guarantee is enabled by these unit tests or Linux cross-compilation, and the candidate tools drive is not serving in Cloud.
...
Candidate publication `0.1.0-tonbo.7-rc.1` was built from `d6e082b35369350e1dea056bda1b7aef0b4dd4f2` by [workflow 35109027659](https://github.com/tonbo-io/AgentENV/actions/runs/35109027659). Independent registry inspection confirms multi-platform digest `sha256:db6c9de8834d7a63accf7027788e3256f382a9c37e86999ee666b850e93d7cf4`, with Linux amd64 manifest `sha256:1d25e2a1e13e7cdc31acc598a4015742a7a883291e9dc4920494de7c227ecbc0` and arm64 manifest `sha256:b5770fadc968f0fbb43efa5cf19ee2fb21f8716560a7d1177612ae28f074a978`. The candidate dependency pin enables isolated runtime qualification; Cloud release-lock promotion remains gated on physical acceptance and consumer integration.
